...
class Storage {
  getBooks = () => {
    let books;
    if (localStorage.getItem('booksSave') === null) {
      books = [];
    } else {
      books = JSON.parse(localStorage.getItem('booksSave'));
    }
    return books;
  };
  ...
  saveBook = (book) => {
    ...
    books.push(book);
    localStorage.setItem('booksSave', JSON.stringify(books));
  };
  ...
  deleteBook = (button) => {
    ...
    localStorage.setItem('booksSave', JSON.stringify(remainBooks));
  };
}
